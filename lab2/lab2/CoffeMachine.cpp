...
namespace
{
	...
	extern const std::string transferFile;
}

void CoffeMachine::showMoney()
{
...
void CoffeMachine::setCommand() const
{
	std::fstream f;
...
void CoffeMachine::proceed()
{
    ...
    else if (argv[0] == std::string("b"))
    {
        ...
        switch (result)
        {
            case 0: out<<("Спасибо за покупку") << std::endl; break;
            case 1: out<<("Недостаточно средств") << std::endl; break;
            case 2: out<<("Извините, данного напитка нет в наличии") << std::endl; break;
			case 3: out<<("Несуществующий ID напитка") << std::endl; break;
			default: out<<("Неизвестная ошибка") << std::endl; break;;
        }
    }
